...
def plotPointsOnImage(img: np.ndarray, points: np.ndarray):
    ...
    plt.title("input image with points at specified coordinates")
...
def homographyFrom4PointCorrespondences(x_d: np.ndarray, x_u: np.ndarray) -> np.ndarray:
    ...
    for n in range(4): # foimport cv2r each reference point generate two lines of the EqSys
...
def main():
    ...
    # Define shape undistorted target image
    M = N = 800
