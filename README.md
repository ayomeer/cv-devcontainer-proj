# Cpp Bindings CUDA Homography Demo
This repository contains a demo application showcasing the use of PyBind11 to incorporate a C++ *CUDA* module into a Python project. The application (python/homography.py) takes an input image of a chess board and applies a homography transformation to perspectively _undistort_ the chessboard, leading to an output image that shows the chess board from directly above.

As a performance reference, the runtime of the undistortion algorithm is printed to the console. You'll note that runtimes for two CUDA kernel launches are reported. This is to show, that the first CUDA kernel launch is always significantly slower because of initialization overhead.
